...
gigabytes, which means that one file block may contain hundreds of logical units whereas another file block only store small
proportion of one logical unit. Then the logical block can not be used as the operation unit. Otherwise, the workflow of each task will be
imbalance if each block contains the same logical units. \par
